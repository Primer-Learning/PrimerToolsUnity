...
namespace Primer.Animation
{
    ...
    public static class IPrimer_ScaleToExtensions
    {
            ...
            [CallerFilePath] string filePath = "", [CallerLineNumber] int lineNumber = 0)
        {
            ...
            return self is IPrimer_CustomScaleTo custom
                ? custom.ScaleTo(Vector3.one * newScale, initial)
                : transform.ScaleTo(Vector3.one * newScale, initial, filePath, lineNumber);
        }
            ...
            [CallerFilePath] string filePath = "", [CallerLineNumber] int lineNumber = 0)
        {
            ...
            return new Tween(
                t => {
                    if (self == null)
                        throw new MissingReferenceException($"Transform is null. Called from {filePath}:{lineNumber}");

                    self.localScale = Vector3.Lerp(from, newScale, t);
                }
            );
        }
    }
