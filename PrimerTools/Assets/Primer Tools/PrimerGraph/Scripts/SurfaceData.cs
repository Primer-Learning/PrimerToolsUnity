...
public class SurfaceData : MonoBehaviour
{
    ...
    public void AnimateToNewSurface(Func<float, float, float> to, float duration = 1.5f, bool ease = true) {
    ...
    /// </summary>
    /// <param name="duration">The duration of the sweep in seconds</param>
    public void AnimateX(float duration = 1.5f, bool ease = true) {
        if (Application.isPlaying) {
            if (animationDone)
    ...
    /// </summary>
    /// <param name="duration">The duration of the sweep in seconds</param>
    public void AnimateY(float duration = 1.5f, bool ease = true) {
        if (Application.isPlaying) {
            if (animationDone)
    ...
    /// </summary>
    /// <param name="duration">The duration of the sweep in seconds</param>
    public void AnimateZ(float duration = 1.5f, bool ease = true) {
        if (Application.isPlaying) {
            if (animationDone)
    ...
    private IEnumerator animateVisibleRange(Vector3 from, Vector3 to, float duration, bool ease) {
        animationDone = false;
        ...
        while (Time.time < startTime + duration) {
            ...
            if (ease) {t = easeInAndOutCubic(t, 0, 1, 1);}
    ...
    public void WipeSurfaceX(float duration = 0.5f, bool ease = true) {
        StartCoroutine(wipeVisibleRange(new Vector3(maxs.x, mins.y, mins.z), duration, ease));
    }
    private IEnumerator wipeVisibleRange(Vector3 final, float duration, bool ease) {
        animationDone = false;
    ...
    private IEnumerator animateSurfaceTransition(Func<float, float, float> from, Func<float, float, float> to, float duration, bool ease) {
        animationDone = false;
    ...
    float easeInAndOutCubic(float t, float startVal, float endVal, float duration)
    {
        ...
        if (t <= 2)
        {
            t -= 2; //Make t negative to use other left side of cubic
