...
public class CurveData : MonoBehaviour
{
    ...
    private string interpolationMode = "none"; //Should probably use an enum, but I forget how that works
    internal Graph plot;
    Vector3[] dataPoints;
    ...
    public void setDataPoints(List<int> points) {
    ...
    IEnumerator AnimatePointsAway(float duration, EaseMode ease) {
        ...
        List<Vector3[]> pointsSets = new List<Vector3[]>();
        foreach (LineRenderer lr in lineRenderers) {
            Vector3[] points = new Vector3[lr.positionCount];
            lr.GetPositions(points); //This is, apparently, how this method works.
            pointsSets.Add(points);
        }
        var init = Time.time;
        for (int i = 0; i < lineRenderers.Count; i++) {
            LineRenderer lr = lineRenderers[i];
            Vector3[] points = pointsSets[i];
            while (Time.time - init < duration) {
                float t = (Time.time - init) / duration;
                t = Helpers.ApplyNormalizedEasing(t, ease);
                float nextPointIndex = t * curveResolution;
                while (curveResolution - lr.positionCount < nextPointIndex) {
                    lr.positionCount -= 1;
